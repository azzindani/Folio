...
* **Editor timeline panel** — PARTLY done. The scrubber now PREVIEWS: it applies the pose to the canvas with recordUndo:false and restores the authored values on stop (it previously moved a thumb and a timecode and changed nothing). `interpolateAtTime` was a second, worse copy of the interpolation — linear only, and blind to skew/blur/draw/scale_x/scale_y — and now delegates to `poseAt`, the same sampler the CSS route and the flipbook use, so all three agree and per-keyframe `easing` is finally honoured. STILL OPEN: a UI control to SET per-keyframe easing (the value is respected but only authorable over MCP), and an `op:sequence` equivalent in the panel. Client changes need a dist rebuild — which works again as of the Dockerfile fix.
