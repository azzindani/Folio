...
| Editor Play / scrub (single page) | `src/editor/motion-pose.ts` (lazy chunk): `layersAt()` over the resolved tree; `MotionPlayer` copies the changed fields into state | every channel, windows, clocks, links, paths — the export's own frame |

`poseAt()` in `keyframe-css.ts` is the reference sampler tests use to check the two agree. A `motion_path` travels on its `easing` in both: `src/animation/path-ease.ts` samples the curve once — SMIL gets the samples as `keyTimes`/`keyPoints` (calcMode linear, `fill="freeze"`, `begin` = the path's `delay`), the flipbook interpolates the same samples. Before this the SVG eased with a hard-coded spline and snapped home at the end while the GIF travelled linearly and held.
...
| Edit | `src/mcp/engine/motion-video-op.ts` | `animation(op:video, layer_id, in?, out?, offset_ms?, duration_ms?, speed?, volume?, muted?, loop?, split_at?)` — the edit a video editor makes: start here, use this part of the file, at this speed, cut there. Moving `offset_ms` without a length keeps the rest of the file; trims are checked against the stored file (an offset past its end is refused, a length past it is trimmed and said). `split_at` cuts the clip in two at a scene time: the first half ends there (`out`, its used length), the second (`<id>_2`) starts there from the frame the first stopped on. Replies with every clip on both clocks — `plays` from–until on the piece, `file` from–to. |
| Editor | `src/editor/canvas-video.ts` | The canvas rebuilds its SVG every render, so each render's `<video>` is swapped for the ONE element kept per layer (moved within the document it keeps playing), and the player drives it: inside its used part it plays at its speed and is re-seeked past 250 ms of drift; otherwise it rests on the playhead's frame — `videoSourceMs`, the moment the export draws. Muted: its sound is CanvasSound's. A lazy chunk, loaded once a design has a video layer; `playsInTime` counts a clip, so a footage-only page shows Play. |
| Search | `src/mcp/engine/asset-video-sources.ts` | `asset_search what:"clip"` (aliases video, footage, b-roll) asks Wikimedia Commons — shortest clips first, each with its own licence and credit line — and NASA's library. `asset_fetch` takes Commons' tallest VP9 WebM at ≤720p (the original is often Ogg Theora, not a stored type) and NASA's smallest MP4, under the video cap (`FOLIO_MAX_VIDEO_BYTES`, 64 MiB), and replies with the video layer to place plus the pointer to `op:video` to trim it. |
...
**In the editor**, an audio asset's right-click menu offers **Use as soundtrack** (`state.setAudioTracks`, undoable; a 1s fade-out by default, since a track cut dead at the end sounds broken). Play all sounds the piece through `src/editor/scene-audio.ts`: the clips of `planSound` scheduled on a WebAudio clock with offsets into decoded buffers and gain ramps for the fades, so it needs no HTTP range support and a seek sounds from the right place. Sound follows the transport's edges — play, the pause-and-play a seek makes, pause — and restarts at the current time after an edit, a finished decode or a mute switch. A file that decodes after play starts late at the position the piece has reached. The stage's sound row (`src/ui/scene-stage/scene-stage-sound.ts`) draws each clip under the scene strip on the same scale, with the plan's notes, a mute switch and each track's volume and fades. A ONE-PAGE piece plays on the canvas, not the stage, and sounds there too: `src/editor/canvas-sound.ts` drives the same `SceneAudio` from the canvas player's transport (start on play, stop on pause, start again from where the clock lands after a loop wrap or a scrub), planned on the export's poster timeline. Its chunk loads as soon as a design has sound, so the audio context starts inside the Play click (found live: a 62 s one-scene piece with a bed and twelve cues played silent in the editor while its mp4 had them all).
...
```
animation(op:storyboard, shots:[{id, at, states:{<id>: {x,y|dx,dy, scale, rotation, opacity, blur, fill.color, enter, exit, loop, loop_ms, hidden, duration, delay, easing} | "hidden" | "show" | "still" | "<preset>"}, duration?, easing?, stagger_ms?, order?, hold?}], length_ms?, hold_ms?)
...
animation(op:retime, at, shift_ms)                              ripple: open (or close) time; everything after moves
...
animation(op:parent, layer_id|layer_ids, to, clear?)          AE parenting: move/turn/scale WITH `to`, about its anchor
animation(op:null, layer_id, x?, y?, width?, height?, layer_ids?)  invisible controller at (x,y); parents layer_ids to it
animation(op:motion_blur, layer_id|layer_ids, shutter?, clear?)   AE motion blur: raster frames smear the layer's travel
...
| States | `src/mcp/engine/motion-states.ts` | A list of absolute states → one track (`origin:offset`). `x`/`y` place the top-left of the DRAWN box after its scale (pivot fractions from the anchor); presets compose onto a state (offsets add, ratios multiply). A layer that starts hidden gets an `in` at its first entrance; one that ends hidden gets an `out`. A change that starts before the previous one lands waits for it (and says so). A state's `loop` (a loop preset, `loop_ms` per pass) is how the layer RESTS: whole passes composed onto the landing pose from where it lands until its next change, unrolled into the same track (a wrapper would pivot where the layer was authored, not where it landed). Alternating presets play back again; a pass that does not end where it starts (spin) holds and restarts 1 ms later. Frames inside it carry `ambient:true` — the players ignore it, the lint reads the loop as ambient, not as moves. At most 120 passes. `"still"` is a change that only stops a loop. |
...
| Lint | `src/mcp/engine/motion-lint.ts` | Each shot is read at its REST — its longest stretch with nothing moving (found live: "just before the next shot" caught a camera already panning away). There: text resting on text (>15% of the smaller box); text CUT by the frame edge, or text that just appeared wholly outside it (text a camera left behind is not flagged); text a shot lands under an opaque layer painted after it (`motion-lint-buried.ts`); OBJECTS the motion cuts into each other (`motion-lint-collide.ts`) — a group is one object unless it holds the scene (the camera, or a group as big as the canvas), leaves as big as the canvas are ground; a pair is flagged when it was apart as authored (≤2% overlap), overlaps 10–90% of the smaller at rest (more is one tucked into the other on purpose) and this shot placed one of them. Text on text skips echoes: the same words at the same size, or a split piece (`split_of`) over a ghost of its whole line — an RGB split is the effect, not a pile-up. Text under an opaque layer, and what a clip hides (`canvasBoxes` intersects `clip`/`clip_rect` windows), is not visible: no overlap, edge or reading check sees it. Wholly-outside text is flagged only when this shot brought it in (itself or a parent moved or entered), so world scenery the camera has yet to reach is not. Reading is a QUEUE: a line is readable when fresh, in frame (≥90%) and drawn ≥ canvas height/40 (smaller is labels and data texture); it lands when its own and its parents' moves end, and a camera move that started before that; the viewer reads lines one at a time in landing order at 240 wpm, and each must be finished before it leaves readable view — which can be shots later. A line the viewer cannot finish is dropped when it leaves, so the next is not blamed. Across the piece: idle stretches >2 s that are longer than the shot's reading time + 1 s; >4 separate things moving at once — siblings changing the same channels over the same time are one gesture (common fate) and every pure fade at a moment is one; link followers do not count; a DRIFT (only x/y/scale, ≤60 px/s and ≤0.05 scale/s, ≥1.5 s — a camera push-in) keeps idle away but does not break a rest; links to missing or still targets. |
| Retime | `motion-retime.ts`, `motion-retime-op.ts` | A ripple edit at `at` (a time or name): `shift_ms`>0 opens that much time, <0 closes the span after it. Every track keyframe/delay/duration, in/out point, path delay, precomp start (inside a precomp that has started it reaches the children on local time; a looping precomp is left alone), marker and `auto_advance` at or after `at` moves; on a one-scene piece so do `audio` starts and caption cues, and a sound with a `duration` playing across it plays on to the new end. A move under way at `at` runs longer and is named (`stretched`) — put `at` in a rest; a move landing exactly at `at` keeps its speed (the time opens after it). A resting loop (`ambient` run) spreads the edit over every pass (found live: one slow gear turn per edit). Closing refuses while anything starts or lands inside the span. |
| Precomp | `src/animation/timeline-resolve.ts` | `clock:{start, speed, loop}` on a group. Inner clocks resolve first; every descendant track, in and out point moves onto the parent clock (`t/speed`, delay `start + delay/speed`). A looping precomp cycles each one-shot child into one period (held at its first pose until its delay, cut at the period). A `motion_path` inside moves onto the parent clock too (`delay` `start + delay/speed`, `duration/speed`; `pathsOnSceneClock` puts the resolved path on the rendered `<animateMotion>`). In a looping precomp a one-shot path runs every pass, like a cycled track: `loop:true` with `period` (the pass) and `offset` (its delay inside the pass) — it waits, travels, rests at the end; `pathAt()` / `pathSMIL()` in `path-ease.ts` are the one timing both players read. In/out inside a looping precomp apply to its first cycle only. |
...
| Motion blur | `export/motion-blur.ts`, `renderer/effects-renderer.ts`, `mcp/engine/motion-blur-op.ts` | `motion_blur: true | {shutter}` (degrees of a frame, default 180) on a layer. Only a sampler given a frame length smears — raster exports pass 1000/fps, op:frame / Play / Play all pass 1000/30; the lint, review and poses pass none. The sampler measures the layer's own pose offset (track + motion path) from t − shutter/2 to t + shutter/2, carried into its local frame under the pose's turn and scale, and sets `effects.motion_blur {dx, dy, samples, box}`; the renderer draws N = 2^k (≤32, ≤3 px apart) feOffset copies averaged PAIRWISE with feComposite arithmetic ½+½ in premultiplied colour — the exact mean of N sub-frames of a move (resvg keeps each result in 8 bits: 24 sequential 1/24 sums reached only 94%), plus a Gaussian one gap long along the travel past the cap, in a userSpaceOnUse region grown by half the travel. Rasterized check: a 200 px bar moving 100 px in a 360° shutter ramps 229 / 127 / 0 at 10 / 50 / 100% coverage. Turns and zooms do not smear. op:motion_blur also flags the wrappers a layer moves through (parent/link/wiggle) and replies with `streaks` (longest smear px at 30 fps, and when). SVG/HTML exports play without it. |
| Parent + null | `motion-rig-op.ts`, `timeline-resolve.ts`, `keyframe-css.ts`, `frame-pose.ts` | `op:parent` = a zero-lag link on x/y/rotation/scale/skew with `link.pivot` = the parent's anchor point as both players measure it (`pivotOfLayer`: its track's `pivot`, else its anchor on its drawn box). The follower's track carries `playback.pivot`, a canvas point: CSS plays it as `transform-box: view-box; transform-origin: Xpx Ypx`, the flipbook and the review's pose map pivot on the same point — so the child ORBITS its parent's anchor instead of turning in place (a plain link pivots on the follower). A parent's own `pivot` is read live; an anchor is measured when parented, and the lint says when the parent has since moved (`Run op:parent again`). `op:null` = a hidden (`visible:false`) empty group whose box is centred on (x,y), animated like any layer. `playback.pivot` is open to op:track too (a clock hand about its hub). Chains (`motion-rig-stack.ts`): a child also gets one mirror level per motion wrapper its parent rides (`<id>_link2`, `<id>_link3`, … innermost first — the parent's own parent link, a follow-through link, a wiggle), each following that wrapper about its pivot, so a grandchild moves with the whole world above it and keeps its place in the paint order; every op:parent (and clear) re-seats all chains until still, and drops mirrors whose child was unparented. A parent linked or wiggled later is caught by the lint (`Run op:parent on … again`). op:frame reports link wrappers with `center` — where the middle of what they draw has gone. |
...
| Editor | `src/editor/motion-pose.ts`, `motion-player.ts`, `src/ui/panels/timeline-track-view.ts`, `timeline-edit.ts` | Play and scrub pose the export's frame (see §1); the timeline draws each row on the scene clock with its window band, motion span, link ghost keys and the page's markers as a Shots strip — and edits them: drag in/out edges, drag / add / rename / remove markers (snapping to markers, the playhead and the ends). The editor's HTML export runs `buildAnimatedSVG` (resolved tree, `lifespans:true`) — it used to ship a still, since the editor never passed `animations`. |

The rescue passes know about time too: `decollideHandPlaced` never moves a layer with an in or out point — two headlines on one spot, one leaving as the other lands, is the composition — and never moves a group (its children carry their own coordinates, so moving the declared box left the drawing behind and the box no longer the pivot of the group's pose); a group is a floor for what is below it instead. But `add_layers` runs before any storyboard exists, so the authoring order is: world first (if any), then the whole scene in ONE `add_layers` call inside a `locked:true` group (the rescue leaves a locked group alone; every motion op still reaches its children by id), then storyboard → precomp / link → camera shots. Found live: an unlocked scene had its cards pushed below the title and its right-hand section snapped onto the canvas.
...
* ~~**Continuous composition**~~ — BUILT 2026-09-18: all six steps (engine + MCP), then the editor (canvas plays the export's frame; timeline on the scene clock with windows, spans, link keys and a Shots strip), the editor HTML export on the resolved tree, loops as a state's rest, `motion_path` on precomp clocks with `delay` and its own easing in both players. Then (same day) the timeline edits in/out points and markers, and a looping precomp repeats a one-shot `motion_path` in both players. Nothing open on this workstream.
