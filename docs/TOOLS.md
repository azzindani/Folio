...
- `scale` (req layer_id|layer_ids + `factor` | `width` | `height` = the size the block should draw at; `anchor` center|top_left|top|top_right|left|right|bottom_left|bottom|bottom_right) — `scaleSubtree` about the anchor: boxes, endpoints, paths, polygons and the lengths with them (font size, stroke, radius, gap). Both reply `before`/`after` boxes, snapshot first, refuse layers inside a LOCKED group (move the group itself, or patch_design).
