...
- `lineage` (req `design_path`; `limit`) — **a design's append-only history**: every change with the tool that made it, an args hash, before/after content hashes, byte sizes and duration. Recorded in `writeYAML` — the single point every design write passes through — so coverage is *structural*: no tool can forget to log, because no tool does the logging. The reply states that scope every time. **Chain breaks** are the real value: a record whose `before` ≠ the previous `after` proves the file was changed outside the tool surface (editor, restored snapshot, sync). Never pruned — a capped audit log is the partial-receipt bug again. Every record is flagged `restorable`.
- `restore` (req `design_path`; `to`, `dry_run`) — **take a change back.** Every mutating tool snapshots before it writes, so ~20 previous states sit in `.mcp_versions/`; this is the only way to read one back, and the reason `heal`/`tokens`/`resize` are safe to attempt unattended. No `to` lists the restore points. Addressing is by **content**: a snapshot is used only when its bytes hash to what that change recorded as its result, so the reply reports `verified:true` — and a pruned state fails loudly, naming the seqs that survive, instead of restoring a neighbour. The rollback is **appended** to the history as its own change and rewrites nothing, so restoring forward again works. Bytes go back verbatim (`writeRaw`) — re-dumping the YAML would change the hash and break the check.
- `style_history` (req `project_path`; `style_seed`, `novelty`, `design_path` to exclude, `limit`) — **what this project already looks like.** Reads every prior design as a STYLE signature — structure (preset + block kinds), composition (columns / vertical band / anchor), palette (ground tone-temperature, accent family, hue count), type scale — with the copy stripped out, because two designs matching on all four ARE the same design with different words. Reports the settled **trait**, not the coarse axis: "6 of 6 are dark-neutral" is actionable where "palette has settled" is also true of a set that changes its accent every time. A trait only counts as evidence when the design could have **chosen** it: for a page that is one full-bleed preset the engine fixes the column measure (always `111`) and the ink centroid (always centre), so both are withheld and named in `not_evidence` rather than silently dropped — otherwise a maximally-varied project reports the same settled list as four designs differing only in their copy, and every seed sends the model to vary something a preset cannot vary. Hand-composed layers get their vote in both back. `novelty` 0 = match the house (a SET is meant to look like a set) · 1 = vary one trait (default) · 2 = vary every settled trait; `style_seed` picks WHICH trait deterministically, so the same seed asks for the same departure twice. `unused` lists readings this project has never produced — coverage of its own catalogue, never a shortlist. `seed_check` compares designs that recorded a `style_seed` (create_design stores it on `meta`): a pair made under different seeds that still signs as one design proves the departure was not taken. It names the trait with room in it and **never the look to put there** (§0.4).
...
- `align` (req layer_ids, operation = left|right|top|bottom|center_h|center_v|distribute_h|distribute_v|snap_grid; `grid`) — the fix for diagnose_design misalignment.
  Moves go through `translateSubtree` (`src/mcp/engine/layer-transform.ts`): a group's children, a path's `d`, a polygon's `points` and a line's endpoints are ABSOLUTE, so moving only the box used to leave the drawing behind while the reply said aligned. A target inside another target moves with it, once.
- `move` (req layer_id|layer_ids + one of `dx`/`dy`, `x`/`y` = the block's new top-left, `to` = center|center_h|center_v on the canvas) — moves the selection **as drawn**, as one block (`src/mcp/engine-transform-tools.ts`). The block is measured by `drawnBox`: a group is where its children draw, not its declared box (a preset group claims the whole canvas). The revise step after `diagnose_design review:true` names the empty area or the off-centre weight. Found live: a revision that moved a group left its children behind.
- `scale` (req layer_id|layer_ids + `factor` | `width` | `height` = the size the block should draw at; `anchor` center|top_left|top|top_right|left|right|bottom_left|bottom|bottom_right) — `scaleSubtree` about the anchor: boxes, endpoints, paths, polygons and the lengths with them (font size, stroke, radius, gap). Both reply `before`/`after` boxes and snapshot first. A layer NAMED inside a LOCKED group is edited like any other and listed under `in_locked_group` (as are update, remove and align) — the lock keeps the engine's heal passes and whole-page ops off the group, not a named edit.
...
- **Req:** `design_path`. **Opt:** `page_id`, `heal`, `max_rounds`, `dry_run`, `review`.
- **`review:true` MEASURES how each page spends its canvas** (`src/mcp/engine/layout-review.ts` + pure `layout-measure.ts`). Each page renders twice at 480 px — the ground alone (full-bleed fills at the bottom, a preset's own backdrop included) and the whole page — and the difference is what a viewer sees on top of the ground, whatever the layer types. Per page: `ink` (share painted), `occupied` (share the content spans, gaps under a 40 px cell included), `content_box` + margins, `empty` (up to three largest empty rectangles, px + share), `balance` (centre of visual weight, offset from centre, left/right + top/bottom split), `thirds` (3×3 occupancy), `components` (largest parts on the canvas and their share — containers opened), `type_scale`, and `notes` for the facts a viewer notices (one empty area ≥20%, weight ≥8% off centre, a content-bearing part ≥40% of the canvas, content spanning <60% of the width). Numbers and places only — no fixes, no taste (§0.4). Off-canvas clips are culled first (the resvg abort). A page that MOVES also gets `motion` (`layout-review-motion.ts`): each shot (its markers, or the whole piece) posed at its rest — the moment the lint judges it (`shotRests` in motion-lint.ts) — and measured like a still (`ink`, `occupied`, `empty`, `balance`, notes), with `rest_ms` (lint's rest: a slow drift does not break it), `still_ms` (nothing moving at all) and `framing_px` (world px across the camera's frame). Time notes: shots never still for 1 s, and ≥60% of shots framed at one size (the slide-deck-scrolled smell). A camera world's authored frame is its top-left, so its `motion.shots` are what the viewer sees; up to 16 shots, one rasteriser batch per page. Each review is kept in `.mcp_versions/<name>.review.json` (`layout-review-compare.ts`): the next `review:true` answers `since_last` per page — deltas of `ink`, `occupied`, `empty_max`, `weight_off` (distance of the weight from centre) and the note KINDS `resolved` / `new` (a note's numbers change with every edit, its kind — empty_area, weight_across, oversized:<id>, `<shot>/never_still` … — does not). Pages not reviewed keep their baseline. The loop: review → `edit_layer move/scale` → review. Found live: a 1920×1080 video's dead space and an off-centre closing card were caught by the user, never by a check.
...
- **`design_echo` / `design_duplicate`** — the new design is a design this project already holds, under another name. Always a *suggestion*, never an error: the engine can see that two designs match but not whether that was the brief, and a lint that calls a deliberate set a defect is the diagnose-that-lies bug in a new costume. Suppressed when lineage proves the copy was deliberate (`duplicate`/`batch`). Not in heal's repairable set — the fix is a different design, and choosing one is yours. `manage_design {op:"style_history"}` shows the whole project's pattern.
...
- Continuous composition (one scene, many sub-sequences — [COMPOSITION.md](COMPOSITION.md)):
  ...
  - `motion_blur` (layer_id|layer_ids, shutter, clear) — After Effects motion blur in every raster frame; replies with each layer's longest streak.
