...
| B17 | Real-device bottom crop: no `viewport-fit=cover` meta → `env(safe-area-inset-*)` always 0 on iPhone; `#app` height 100% tracks the LARGE viewport under browser chrome; statusbar (zoom bar) had no safe-area padding and grid rows pinned it — bottom controls rendered under the URL bar / home indicator | high | ✓ fixed 2026-07-10: viewport-fit=cover + `#app{height:100dvh}` (mobile+tablet) + statusbar auto-row w/ env() padding + mobile-nav & bottom-sheet offsets include the inset |
| B18 | Tablet: ~36px sliver of the parked left panel (toolbox edge) floats over the canvas and first page-strip thumbnail; open overlay also sat 48px too far right. Cause: abspos grid items anchor to their GRID AREA, not #app — `left: var(--activity-bar-width)` double-counted the 0-width `lpanel` column offset, and `translateX(-105%)` of the panel's own width couldn't clear it | medium | ✓ fixed 2026-07-10: `left: 0` in the tablet block; multipage audit re-verified clean at 820×1180 |
| B19 | Right-panel DOCUMENT "Page" row goes stale on page switch/add (kept saying "Cover (1/3)" while canvas showed page 2; after Add Page it showed the wrong label) — properties panel only re-rendered on `selectedLayerIds`/`design`, never `currentPageIndex` | medium | ✓ fixed 2026-07-10: `currentPageIndex` added to the watched keys |
| B20 | Mobile: fixed bottom nav painted over the last 52px of the page strip — thumb labels + the "+" (add page) button hidden behind it on every multipage design | medium | ✓ fixed 2026-07-10: `#page-strip-section{margin-bottom:var(--mobile-nav-height)}` in the mobile block |
| B21 | Layers panel showed `?` icons for valid layer types missing from LAYER_ICONS (ellipse, polyline, connector, background, backdrop, particle, map, embed_code, component_list, auto_layout) | low | ✓ fixed 2026-07-10: icon map completed against the validator's VALID_LAYER_TYPES |

Multipage editor pass (2026-07-10, desk 1600 / tab 820 / mob 390, live): page strip render+labels, switch, thrash ×8, add/duplicate/delete/undo, gradient defs across switches (0 dangling url(#) refs), payload view, presentation mode — all green after B18–B21. Rig: `uxaudit/multipage.mjs` + `fixture-pages.yaml` (3 gradient pages).

Chrome polish (2026-07-10): every editor-chrome emoji (📁 ⭐ 🎨 ♿ ✋ 🖼 💉 …) replaced with a shared monoline stroke-SVG set on currentColor — `src/editor/chrome-icons.ts` — across the left/right activity bars, mobile nav, toolbox, file tree, catalog chips and theme toggle. Page-strip thumbnail well now `var(--color-surface-2)` instead of hard-coded white (bg-less designs no longer flash white thumbs against the dark canvas). Verified live: desktop dark/light, files panel, mobile — consistent flat icons, active-state tinting, zero page errors.
