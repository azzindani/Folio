...
| [COMPOSITION.md](COMPOSITION.md) | Continuous composition — one scene with layered sub-sequences instead of slide scenes; positioning (declarative 2D vs HTML video, leverage points, why no 3D), the state-based `op:storyboard` protocol, what was built (storyboard, markers, span, precomp, link, lint, world camera) and what is open | …you're building a continuous video piece or picking up the editor side |
