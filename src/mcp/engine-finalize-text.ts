...
import type { Layer, Page, ThemeSpec } from '../schema/types';
...
export function fixInvisibleText(layers: Layer[], docW: number, docH: number, theme?: ThemeSpec): number {
  ...
  let fixed = 0;
  for (const l of flat) {
    if (l.type !== 'text' || !layerText(l).trim()) continue;
    const o = l as unknown as Record<string, unknown>;
    const st = (o['style'] as Record<string, unknown>) ?? {};
    ...
    if (!effHex) continue;          // unevaluable (unknown token) → leave it
    const cr = crRatio(effHex, bg);
    if (cr === null || cr >= MIN_TEXT_CR) continue;   // already legible
    ...
    if (flatCol && flatCol !== eff) {
      ...
      const fcr = flatHex ? crRatio(flatHex, bg) : null;
      if (fcr !== null && fcr >= MIN_TEXT_CR) next = flatCol;  // keep original (may be a token)
    }
    // 2. Else force a neutral matched to the backdrop.
    if (!next) next = bgLum < 0.5 ? '#FAFAFA' : '#141414';
