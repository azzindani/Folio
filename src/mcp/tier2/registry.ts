...
export const TIER2_TOOLS: ToolDefinition[] = [
  ...
  {
    name: 'edit_layer',
    description: 'Edit individual layers on a sealed/in-progress design — pick `op` (for BULK composition use add_layers instead):\n• add — add ONE layer (req: design_path, layer). Sized layers need width+height.\n• update — merge props into a layer by ID (req: design_path, layer_id, props). Snapshots before write.\n• remove — delete a layer by ID (req: design_path, layer_id).\n• align — auto-align/distribute/snap a set of layers, the fix for diagnose_design misalignment (req: design_path, layer_ids, operation = left|right|top|bottom|center_h|center_v|distribute_h|distribute_v|snap_grid).\n• move — move layers AS DRAWN, children and paths with them, as one block (req: design_path, layer_id|layer_ids + one of: dx/dy · x/y = the block\'s new top-left · to:"center"|"center_h"|"center_v" on the canvas). The revise step after diagnose_design review:true names the empty area or the off-centre weight.\n• scale — scale layers as one block about an anchor, type/strokes/gaps included (req: design_path, layer_id|layer_ids + factor | width | height = the size the block should draw at; anchor: center (default)|top_left|top|top_right|left|right|bottom_left|bottom|bottom_right).\n• patch_spec — change a PRESET by editing the spec that built it, then re-render it in place (req: design_path, layer_id, changes). A preset group is ~30 generated layers; patching the intent (\"three blocks not five\", a new accent, a different headline_style) is one call instead of thirty edits or a rebuild. Objects merge key by key, ARRAYS replace wholesale, null DELETES a field. Read the current spec first with manage_design {op:\"get_spec\"}; pass dry_run:true to see the diff without writing.\nCAROUSEL: pages share layer IDs (sections_1 etc.) — pass page_id to scope the edit to ONE page. Without it, an id that exists on several pages is REFUSED rather than applied to all of them or guessed at, and the error names the pages it found.\n• detach — bake a RULE into what it draws, for hand editing (opt: layer_id or layer_ids; page_id). A layer\'s source formulas become their values, a gallery becomes its literal cells (<id>_<n>), an animation.rule becomes its keyframes — drawn exactly as before. No ids = every rule on the page (page_id) or in the whole design: the file then stores its full expansion.',
    inputSchema: {
      type: 'object',
      properties: {
        op:           { type: 'string', enum: ['add', 'update', 'remove', 'align', 'move', 'scale', 'patch_spec', 'shape', 'split_text', 'detach'], description: 'Which layer edit to run.' },
        design_path:  { type: 'string', description: 'Path to .design.yaml.' },
        ...
        layer_id:     { type: 'string', description: 'op:update/remove/patch_spec/move/scale — the layer ID.' },
        by:           { type: 'string', enum: ['char', 'word'], description: 'op:split_text — split a text layer into one layer PER CHARACTER (default) or per word, each placed at its measured advance, so animation {op:"sequence", stagger_ms} can reveal them one at a time. Single-line layers only. Pieces are placed from the real font metrics when the family is bundled; the reply says which, because a drifting run and an unbundled font look the same.' },
        ...
        dx:           { type: 'number', description: 'op:move — px to move right (negative = left).' },
        dy:           { type: 'number', description: 'op:move — px to move down (negative = up).' },
        x:            { type: 'number', description: 'op:move — the block\'s new left edge.' },
        y:            { type: 'number', description: 'op:move — the block\'s new top edge.' },
        to:           { type: 'string', enum: ['center', 'center_h', 'center_v'], description: 'op:move — centre the block on the canvas (both axes, across, or down).' },
        factor:       { type: 'number', description: 'op:scale — scale factor (0.05–20).' },
