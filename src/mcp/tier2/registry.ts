...
export const TIER2_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        op:           { type: 'string', enum: ['add', 'update', 'remove', 'align', 'patch_spec', 'shape', 'split_text'], description: 'Which layer edit to run.' },
        design_path:  { type: 'string', description: 'Path to .design.yaml.' },
        ...
        width:        { type: 'number', description: 'op:shape/outline_stroke — the stroke width to outline, px. Defaults to the layer\'s own stroke width. Undeclared, a conforming client dropped it and the op could not be called at all.' },
        ...
        layer_ids:    { type: 'array', description: 'op:align — layer IDs to align. op:shape — the path layer to operate on, or the TWO paths to blend between.', items: { type: 'string' } },
