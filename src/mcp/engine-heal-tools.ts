...
export function healDesign(args: {
  ...
  for (let r = 1; r <= rounds; r++) {
    ...
    for (const [n, what] of [[t.nulls, 'null layer(s) dropped'], [t.recovered, 'JSON-in-text layer(s) recovered'], [t.placeholders, `placeholder text layer(s) deleted (${t.placeholderText.join(', ')})`], [t.placed, 'positionless layer(s) flowed'], [t.bgFilled, 'background filled'], [t.reflowed, 'overlapping layer(s) reflowed'], [t.snapped, 'layer(s) pulled back onto the canvas'], [t.relit, 'invisible text re-lit']] as [number, string][]) {
