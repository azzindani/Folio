 ...
 *
 * `rotation` turns the world under the camera, about that same centre (both
 * players rotate and scale about one anchor), so the offset carries the TURNED
 * delta: T = C − P − s·R(c − P). Without it a tilted camera framed the wrong
 * place, which is why a dive could only be tilted by leaning its content.
 */
export function framePose(target: Box, canvas: { width: number; height: number }, padding = 0, pivot?: { x: number; y: number }, rotation = 0): CameraPose {
  ...
  return { scale: Number(s.toFixed(4)), x: r2(canvas.width / 2 - P.x - s * rx), y: r2(canvas.height / 2 - P.y - s * ry) };
}
