...
import { inspectTimeline } from '../engine-runtime-tools';
...
describe('scenes across timeline, frame and export', () => {
  ...
  it('gives a piece moved only by rules, loops and scripts a length every reader keeps, and retime moves it', () => {
    ...
    const tl = inspectTimeline({ design_path: lab }) as unknown as { scene_ms: number; ascii: string };
    expect(tl.scene_ms).toBe(6000);
    const row = (id: string): string => tl.ascii.split('\n').find(l => l.startsWith(id)) ?? '';
    expect(tl.ascii.split('\n')[0], 'drawn to the same length').toMatch(/^Scene 6000ms/);
    expect(row('blob'), 'a script component has its row, to the end').toMatch(/░\| drawn$/);
    expect(row('mug'), 'an endless loop runs to the end').toMatch(/∞\| /);
