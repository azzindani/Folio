...
// ── Color math (pure) ─────────────────────────────────────────
// Lives in ./color-math — a leaf, so a module can reason about colour without
// importing this tool (and the write path it reaches). Re-exported here because
// every existing caller imports it from reference.
import { toHex, luminance, saturation, hue, mix, dedupeColors, type RGB } from './color-math';
export {
  hexToRgb, toHex, luminance, saturation, hue, mix, dedupeColors,
} from './color-math';
