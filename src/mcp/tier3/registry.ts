  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        type:         { type: 'string', enum: ['svg', 'html', 'gif', 'mp4', 'webm'], description: 'op:export — output format. svg = self-contained animated SVG (no dependencies, vector). html = the same SVG in a shareable single file that honors prefers-reduced-motion. gif = raster flipbook encoded IN-PROCESS (pure TS + resvg — no ffmpeg, no Puppeteer); the format to use for a feed that will not play an SVG. mp4/webm DO need Puppeteer + ffmpeg on the host and error clearly where they are absent.' },
        all_pages:    { type: 'boolean', description: 'op:export — a carousel/deck writes ONE animated file per page (`<name>-p1.gif`, `-p2.gif`, …) instead of just the first; output_paths lists them. Use it for a per-page motion companion to a PDF carousel.' },
