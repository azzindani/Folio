  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        clear:        { type: 'boolean', description: 'op:motion_path — remove the path from the named layers instead of setting one. op:captions — remove captions (with page_id: that scene\'s only).' },
        ...
        shots:        { type: 'array', items: { type: 'object', properties: {} }, description: 'op:camera — [{t:ms, target:"all"|id|[ids], padding?:px, easing?, hold?}]. Each shot frames its target to fill the canvas; the camera eases between shots. e.g. [{t:0,target:"all"},{t:1200,target:"stat",padding:80},{t:3000,target:"all"}].' },
        ...
        to:           { type: 'string', description: 'op:morph — the outline to become, as an SVG `d` string (no elliptical arcs).' },
        ...
        hold_ms:      { type: 'number', description: 'op:export/frame/timeline with scenes — rest after each scene\'s motion, ms (default 1500; a page\'s length_ms wins).' },
        ...
        length_ms:    { type: 'number', description: 'op:scene — exact time the page is on screen, ms. 0 clears it (motion + hold).' },
